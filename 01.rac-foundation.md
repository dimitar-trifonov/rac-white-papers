...
This white paper defines the core philosophy, structure, and practical foundations of **Requirements-as-Code (RaC)** — a declarative, testable, and technology-agnostic approach that bridges human intent with AI-assisted software development. By shifting from ephemeral prompts to persistent structures, RaC introduces a durable layer of logic that enables simulation, collaboration, and seamless integration across the software development lifecycle (SDLC). RaC is not just a coding methodology — it is a **general-purpose expression framework** for building systems that are understandable by both humans and machines.
...
### 4.1 RaC/SaC as a General-Purpose Language of Systems

RaC (Requirements-as-Code) and its superset SaC (Systems-as-Code) aim to provide a general-purpose, declarative language for systems. In the same way that mathematics gives us a shared language for quantities, RaC/SaC offers a shared, declarative way to describe systems and processes: a schema-driven layer where intent is declared once, simulated, audited, and bound to multiple runtimes.

- **Primitives**
  - State — what exists now
  - Events — what can happen
  - Logic — what must hold or be forbidden
  - Tests — how to validate behavior
  - UI/API — how humans and machines interact
- **Dual-mode execution**
  - Online: AI/MCP orchestrators execute against provider implementations
  - Offline: deterministic playbooks humans can follow
- **Architecture layers**
  1. Specification (RaC) — declarative YAML across `state/`, `events/`, `logic/`, `tests/`, `ui/`, `bindings/`, `schemas/`
  2. Binding — map RaC units to runtimes (React, Firebase, SQL/GenKit, APIs)
...
- **Position vs adjacent tools**
  - BPMN/UML: accessible but weakly testable and non-executable
  - Terraform/K8s (IaC): declarative but infra-bound
  - OPA/Rego: strong policy, narrow scope
  - SysML/TLA+: rigorous, expert-only
  - RaC/SaC: lightweight, schema-driven, human+AI friendly, testable, auditable, tech-agnostic

Thesis: A broad class of processes expressible in RaC can be simulated, validated, and operationalized as services.
